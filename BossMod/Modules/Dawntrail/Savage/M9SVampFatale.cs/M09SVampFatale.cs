...
// Can't seem to make this wait for the BrutalRain1 casts and then vanish, so erring on the side of at least getting the stack in place for the first hit and hoping people will stay.
sealed class BrutalRain(BossModule module) : Components.StackWithIcon(module, (uint)IconID.BrutalRain, (uint)AID.BrutalRain, 6f, 0d);

sealed class UltrasonicSpread(BossModule module) : Components.BaitAwayCast(module, aid: (uint)AID.UltrasonicSpread, shape: new AOEShapeCone(60f, 15f.Degrees()), endsOnCastEvent: true);
sealed class UltrasonicSpreadTankbuster(BossModule module) : Components.BaitAwayCast(module, aid: (uint)AID.UltrasonicSpread2, shape: new AOEShapeCone(60f, 45f.Degrees()), endsOnCastEvent: true, tankbuster: true);
sealed class UltrasonicAmp(BossModule module) : Components.Adds(module, (uint)OID.UltrasonicAmp);

sealed class CharnelCells(BossModule module) : Components.Adds(module, (uint)OID.CharnelCell);

sealed class DeadlyDoornail(BossModule module) : Components.Adds(module, (uint)OID.DeadlyDoornail);
sealed class FatalFlail(BossModule module) : Components.Adds(module, (uint)OID.FatalFlail);
...
sealed class HellInACell(BossModule module) : Components.CastTowers(module, (uint)AID.HellInACell, 3f);

sealed class FinaleFatale(BossModule module) : Components.RaidwideCast(module, (uint)AID.FinaleFatale);
sealed class Enrage(BossModule module) : Components.RaidwideCast(module, (uint)AID.FinaleFatale2, hint: "Enrage");
...
sealed class AetherlettingCross(BossModule module) : Components.SimpleAOEs(module, (uint)AID.Aetherletting2, new AOEShapeCross(40f, 5f));
...
Contributors = "HerStolenLight, Xaenalt",
