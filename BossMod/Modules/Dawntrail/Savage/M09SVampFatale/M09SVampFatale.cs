...
//sealed class Hardcore(BossModule module) : Components.BaitAwayCast(module, (uint)AID.Hardcore, 6f);
//sealed class HardcoreBig(BossModule module) : Components.BaitAwayCast(module, (uint)AID.HardcoreBig, 15f);
...
// 3 if regular but not always, sometimes 4; 5 hits when 8 stacks? 1 more cast each time it's used?; stack based, 5 hits on 2nd cast with 8 satisfied stacks
//sealed class BrutalRain(BossModule module) : Components.StackWithIcon(module, (uint)IconID.ShareMulti, (uint)AID.BrutalRain, 6f, 5.1d, 6, 8, 3);
...
sealed class Coffinfiller(BossModule module) : Components.SimpleAOEGroups(module, [(uint)AID.CoffinfillerLong, (uint)AID.CoffinfillerMed, (uint)AID.CoffinfillerShort], new AOEShapeRect(32f, 2.5f), 2, riskyWithSecondsLeft: 2d);
sealed class DeadWake(BossModule module) : Components.SimpleAOEs(module, (uint)AID.DeadWake, new AOEShapeRect(10f, 10f), riskyWithSecondsLeft: 2d);
// change risky time only during Arena1 for AI melee uptime
