...
sealed class Hole(BossModule module) : BossComponent(module)
{
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00040008u)
        {
            ...
            {
                Arena.Bounds = new ArenaBoundsCustom(D060BiocultureNode.GetVertices1());
            }
            else if (actor.OID == (uint)OID.Boss2Hole)
            {
                Arena.Bounds = new ArenaBoundsCustom(D060BiocultureNode.GetVertices2());
            }
        }
    }
}

sealed class Puncture(BossModule module) : Components.SimpleAOEs(module, (uint)AID.Puncture, new AOEShapeRect(5.5f, 1.5f));
sealed class TailSlap(BossModule module) : Components.SimpleAOEs(module, (uint)AID.TailSlap, new AOEShapeCone(7f, 60f.Degrees()));
sealed class CalcifyingMist(BossModule module) : Components.SimpleAOEs(module, (uint)AID.CalcifyingMist, new AOEShapeCone(6.9f, 45f.Degrees()));
sealed class EerieSoundwave(BossModule module) : Components.SimpleAOEs(module, (uint)AID.EerieSoundwave, 7.5f);

sealed class MarrowDrain(BossModule module) : Components.SimpleAOEGroups(module, [(uint)AID.MarrowDrain1, (uint)AID.MarrowDrain2, (uint)AID.MarrowDrain3], new AOEShapeCone(9.7f, 60f.Degrees()));

sealed class TheRamsVoice(BossModule module) : Components.SimpleAOEs(module, (uint)AID.TheRamsVoice, 9.7f);
sealed class Sideswipe(BossModule module) : Components.SimpleAOEs(module, (uint)AID.Sideswipe, new AOEShapeCone(9f, 45f.Degrees()));
sealed class Gust(BossModule module) : Components.SimpleAOEs(module, (uint)AID.Gust, 3f);

sealed class D060BiocultureNodeStates : StateMachineBuilder
{
    public D060BiocultureNodeStates(BossModule module) : base(module)
    {
        TrivialPhase()
            ...
            .Raw.Update = () => AllDeadOrDestroyed(D060BiocultureNode.IsArena1(Module.PrimaryActor) ? D060BiocultureNode.Trash1 : D060BiocultureNode.Trash2);
    }
}

[ModuleInfo(BossModuleInfo.Maturity.Verified, Contributors = "The Combat Reborn Team (Malediktus)", GroupType = BossModuleInfo.GroupType.CFC, GroupID = 38u, NameID = 3830u, SortOrder = 5)]
public sealed class D060BiocultureNode : BossModule
{
    public D060BiocultureNode(WorldState ws, Actor primary) : this(ws, primary, IsArena1(primary) ? BuildArena1() : BuildArena2()) { }

    private D060BiocultureNode(WorldState ws, Actor primary, (WPos center, ArenaBoundsCustom arena) a) : base(ws, primary, a.center, a.arena) { }

    public static bool IsArena1(Actor primary) => primary.PosRot.Z < 250f;

    public static PolygonCustom[] GetVertices1()
    {
        return [(new PolygonCustom([new(26.19f, 163.9f), new(29.99f, 164.07f), new(30.59f, 164.12f), new(31.3f, 164.26f), new(31.59f, 164.85f),
    ...
    public static PolygonCustom[] GetVertices2()
    {
        return [(new PolygonCustom([new(119.47f, 228.45f), new(119.98f, 228.57f), new(119.98f, 245.6f), new(120.27f, 246.05f), new(125.43f, 251.12f),
            ...
            new(99.85f, 294.15f), new(100f, 293.58f), new(101.46f, 292.12f), new(102.05f, 291.77f), new(102.03f, 291.14f),
    ...
    {
        var arena = new ArenaBoundsCustom(GetVertices1(), [new Polygon(new(28.122f, 220.05f), 2.5f, 16)]);
        return (arena.Center, arena);
    }

    private static (WPos center, ArenaBoundsCustom arena) BuildArena2()
    {
        var arena = new ArenaBoundsCustom(GetVertices2(), [new Polygon(new(111.925f, 271.931f), 2.5f, 16)]);
        return (arena.Center, arena);
    }
