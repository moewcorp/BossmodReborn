...
sealed class PassiveInfraredGuidanceSystem(BossModule module) : Components.Cleave(module, (uint)AID.PassiveInfraredGuidanceSystem, new AOEShapeCircle(6f), [(uint)OID.RetooledEnforcementDroid], originAtTarget: true);
sealed class TheHand(BossModule module) : Components.SimpleAOEs(module, (uint)AID.TheHand, new AOEShapeCone(7.5f, 60f.Degrees()));
sealed class Shred(BossModule module) : Components.SimpleAOEs(module, (uint)AID.Shred, new AOEShapeRect(5.5f, 2f));
sealed class SpawnReservoir(BossModule module) : Components.SimpleAOEs(module, (uint)AID.SpawnReservoir, 3f);
sealed class AutoCannons(BossModule module) : Components.SimpleAOEs(module, (uint)AID.AutoCannons, new AOEShapeRect(42f, 2.5f));

sealed class D080CrimsonControlUnitStates : StateMachineBuilder
{
...
public sealed class D080CrimsonControlUnit : BossModule
{
    public D080CrimsonControlUnit(WorldState ws, Actor primary) : this(ws, primary, BuildArena()) { }

    private D080CrimsonControlUnit(WorldState ws, Actor primary, (WPos center, ArenaBoundsCustom arena) a) : base(ws, primary, a.center, a.arena) { }

    private static (WPos center, ArenaBoundsCustom arena) BuildArena()
    {
        PolygonCustom[] poly = [new PolygonCustom([new(-92.05f, -480.33f), new(-81.49f, -479.48f), new(-80.99f, -479.05f), new(-78.48f, -473.69f), new(-78.13f, -473.22f),
        ...
        var arena = new ArenaBoundsCustom(poly, [new Rectangle(new(-126.383f, -423.171f), 1.39f, 0.9f, 60f.Degrees()),
    ...
    public override bool CheckReset() => !Arena.InBounds(Raid.Player()!.Position);

    protected override bool CheckPull() => Arena.InBounds(Raid.Player()!.Position) && IsAnyActorInCombat(trash);
