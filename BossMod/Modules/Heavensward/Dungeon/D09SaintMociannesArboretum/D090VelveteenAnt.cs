...
{
    private bool isArena1B;
    private bool isArena2;

    private static ArenaBoundsCustom GetArena1B() => new([D090VelveteenAnt.GetVertices1(), D090VelveteenAnt.GetVertices2()]);
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00040008u)
        {
            ...
            else if (actor.OID == (uint)OID.WallController2)
            {
                isArena2 = false;
                isArena1B = false;
                SetArena(new([D090VelveteenAnt.GetVertices3()]));
            }
        }
    }
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        ...
        {
            var pZ = pos.Z;
            if (isArena1B && pZ < -186f)
            {
                isArena2 = true;
                isArena1B = false;
                SetArena(new([D090VelveteenAnt.GetVertices2()]));
            }
            else if (isArena2 && pZ > -186f)
            {
                isArena2 = false;
                isArena1B = true;
                SetArena(GetArena1B());
...
sealed class D090VelveteenAntStates : StateMachineBuilder
{
    public D090VelveteenAntStates(BossModule module) : base(module)
    {
        TrivialPhase()
            ...
            .Raw.Update = () => AllDeadOrDestroyed(D090VelveteenAnt.All) || Module.Raid.Player()!.Position.InCircle(new(-268f, -134f), 20f);
    }
}

[ModuleInfo(BossModuleInfo.Maturity.Verified, Contributors = "The Combat Reborn Team (Malediktus)", GroupType = BossModuleInfo.GroupType.CFC, GroupID = 41u, NameID = 4641u, SortOrder = 3)]
public sealed class D090VelveteenAnt : BossModule
{
    public D090VelveteenAnt(WorldState ws, Actor primary) : this(ws, primary, BuildArena()) { }

    private D090VelveteenAnt(WorldState ws, Actor primary, (WPos center, ArenaBoundsCustom arena) a) : base(ws, primary, a.center, a.arena) { }

    public static PolygonCustom GetVertices1()
    {
        return new PolygonCustom([new(-123.8f, -191.08f), new(-117.35f, -190.85f), new(-116.85f, -190.4f), new(-115.74f, -188.76f), new(-113.76f, -186.84f),
    ...
    {
        return new PolygonCustom([new(-209.62f, -227.66f), new(-208.3f, -227.35f), new(-207.71f, -226.23f), new(-207.36f, -225.67f), new(-206.79f, -225.67f),
    ...
    public static PolygonCustom GetVertices3()
    {
        return new PolygonCustom([new(-237.07f, -188.28f), new(-236.44f, -188.22f), new(-235.81f, -188.08f), new(-235.23f, -187.88f), new(-234.66f, -187.65f),
    ...
    {
        var arena = new ArenaBoundsCustom([GetVertices1()]);
