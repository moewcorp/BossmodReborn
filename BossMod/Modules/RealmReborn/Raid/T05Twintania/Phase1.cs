...
// P1 mechanics
class P1LiquidHellAdds(BossModule module) : Components.VoidzoneAtCastTarget(module, 6, (uint)AID.LiquidHellAdds, m => m.Enemies((uint)OID.LiquidHell).Where(z => z.EventState != 7), 0); // note: voidzone appears ~1.2s after cast ends, but we want to try avoiding initial damage too
...
class P1AI(BossModule module) : BossComponent(module)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            hints.AddForbiddenZone(new SDInvertedCircle(Module.PrimaryActor.Position, 8), DateTime.MaxValue);
        }
    }
}
