...
sealed class Ex3TitanAIRotation(RotationModuleManager manager, Actor player) : AIRotationModule(manager, player)
{
    ...
    {
        MovementStrategy.Pathfind => NavigationDecision.Build(NavigationContext, World.CurrentTime, Hints, Player.Position, Player.CastInfo, Speed()).Destination ?? Player.Position,
