...
public sealed class UIPresetEditor
{
    ...
    public void DrawModulesList()
    {
        ...
        if (UIMisc.Button("移除##module", width.X, (_selectedModuleIndex < 0, "请先选择要移除的模块"), (!ImGui.GetIO().KeyShift, "按住 Shift")))
        {
    ...
    private void DrawSettingsList()
    {
        ...
        {
            ImGui.TextUnformatted("请添加或选择循环模块以配置其策略");
            return;
        }
        ...
        using (var combo = ImRaii.Combo("###selectover", "添加新覆盖..."))
        {
            if (combo)
            {
    ...
    private bool SuggestHealerAI(Preset.ModuleSettings ms)
    {
        if (!_currentModuleHasHealerAI && _autorotConfig.SuggestHealerAI && _misleadingHealerRotations.Contains(ms.Type))
        {
            ...
            ImGui.TableNextColumn();
            ImGui.TextUnformatted("不属于标准循环。请改用治疗AI模块。");
            if (ImGui.Button("添加治疗AI"))
            {
