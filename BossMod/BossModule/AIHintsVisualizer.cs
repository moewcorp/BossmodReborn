...
public sealed class AIHintsVisualizer(AIHints hints, WorldState ws, Actor player, float preferredDistance, float cushionSize)
{
    ...
    public void Draw(UITree tree)
    {
        ...
        var countF = hints.ForbiddenZones.Count;
        foreach (var _1 in tree.Node("Forbidden zones", countF == 0))
        {
            for (var i = 0; i < countF; ++i)
            {
                var zone = hints.ForbiddenZones[i];
                foreach (var _2 in tree.Node($"[{i}] activated at {Math.Max(0d, (zone.activation - ws.CurrentTime).TotalSeconds):f3}"))
                {
                    (_zoneVisualizers[i] ??= BuildZoneVisualizer(zone.shapeDistance)).Draw();
                }
            }
        }
        var off = countF;
    ...
    private MapVisualizer BuildGoalVisualizer(Func<WPos, float> shape)
    {
        var map = new Map();
        hints.InitPathfindMap(map);
        NavigationDecision.RasterizeGoalZones(map, [shape]);
        return new MapVisualizer(map, player.Position);
    }
